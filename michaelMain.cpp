...
vector<Course> parseCSV(string fileName){
	...
	while(getline(inFile,line)){
		stringstream ss;
		ss << line;
		string title, course_major,course_num, section, instructor, day, time, building, room;
		// Get title
		if (ss.peek() == '"') {
			ss.ignore();
			getline(ss, title, '"');
		...
		Course currentCourse = Course(title,course_major,course_num,section,instructor,day,timeArr[0],timeArr[1],building,room);
